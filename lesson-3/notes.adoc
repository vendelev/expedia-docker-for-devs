...
include::docfiles/sharedVolume[lines=1..4]
...
Several ports can be exposed either by using the `EXPOSE` instruction several times or using `EXPORT <port1> <port2> ....`
...
[TIP]
--
To install nginx, use those commands: `apt-get update` and `apt-get install -y ngnix`.

To start the web server: `nginx -g 'daemon off;'`
--

include::solutions.adoc[tags=customNginx]
>>>>>>> Add lesson-3 - exercises only
